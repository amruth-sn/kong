...
def _recover_modulo(code: str) -> str:
    ...
    while changed:
        ...
        for match in tail_pattern.finditer(code):
            ...
            suffix_sub = " - "
            if prefix.endswith(suffix_sub) and prefix[: -len(suffix_sub)].endswith(
                inner_expr
            ):
            ...
            if (
                has_neg_mult
                and prefix.endswith(suffix_add)
                and prefix[: -len(suffix_add)].endswith(inner_expr)
            ):
...
def _is_accumulator(code: str, var: str) -> bool:
    ...
    return bool(
        init_zero.search(code) and (add_assign.search(code) or add_self.search(code))
    )
...
def _is_pointer_like(code: str, var: str) -> bool:
    ...
    return bool(
        null_cmp.search(code) or dat_assign.search(code) or ptr_cast_deref.search(code)
    )
